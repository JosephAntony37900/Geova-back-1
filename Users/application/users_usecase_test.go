...
func BenchmarkLogin(b *testing.B) {
	...
	for i := 0; i < b.N; i++ {
		_, _, err := useCase.Execute(email, password)
		if err != nil {
...
// BenchmarkLogin_WithoutComparison mide todo EXCEPTO la comparación de Bcrypt
// (comentado porque validateEmail y validatePassword son privados)
/*
func BenchmarkLogin_WithoutComparison(b *testing.B) {
	mockRepo := NewMockUserRepository()
	bcryptService := adapters.NewBcrypt()
	mockJWT := &MockTokenManager{}
	useCase := NewLoginUseCase(mockRepo, mockJWT, bcryptService)
	...
	for i := 0; i < b.N; i++ {
		_ = useCase.validateEmail(email)
		_ = useCase.validatePassword(password)
	}
}
*/
